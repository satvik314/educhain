...
class AudioProcessor:
    ...
    **kwargs
        ...
        try:
            ...
            # --- MODEL ---
            model = model or "ResembleAI/chatterbox-turbo"
            ...
            print(f"[DeepInfra] Requesting TTS: {text[:60]}...")
            ...
            print("[DeepInfra] Response received.")

            audio_field = result.get("audio")
            if not audio_field:
                raise Exception("No audio field in DeepInfra response")

            # --- HANDLE DATA URI ---
            if audio_field.startswith("data:audio"):
                base64_audio = audio_field.split(",", 1)[1]
            else:
                # fallback (pure base64)
                base64_audio = audio_field

            # --- DECODE BASE64 ---
            audio_bytes = base64.b64decode(base64_audio)

            if not audio_bytes:
                raise Exception("Decoded audio is empty")
